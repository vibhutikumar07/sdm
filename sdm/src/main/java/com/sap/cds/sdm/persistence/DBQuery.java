...
public class DBQuery {
  ...
  public static Result getUrlforAttachmentId(
      CdsEntity attachmentEntity, PersistenceService persistenceService, String Id) {
    CqnSelect q = Select.from(attachmentEntity).columns("url").where(doc -> doc.get("ID").eq(Id));
    return persistenceService.run(q);
  }

  public static Result isFolderExisting(
      CdsEntity attachmentEntity, PersistenceService persistenceService, String folderId) {
    CqnSelect q = Select.from(attachmentEntity).where(doc -> doc.get("folderId").eq(folderId));
    return persistenceService.run(q);
  }
